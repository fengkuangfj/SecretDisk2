...
#define IOCTL_UM_DIR_DELETE		CTL_CODE(SIOCTL_TYPE, 0xC03, METHOD_BUFFERED, FILE_ANY_ACCESS)	// 4000300c
#define IOCTL_UM_DIR_CLEAR		CTL_CODE(SIOCTL_TYPE, 0xC04, METHOD_BUFFERED, FILE_ANY_ACCESS)	// 40003010
#define IOCTL_UM_PROC_ADD		CTL_CODE(SIOCTL_TYPE, 0xC05, METHOD_BUFFERED, FILE_ANY_ACCESS)	// 40003014
#define IOCTL_UM_PROC_DELETE	CTL_CODE(SIOCTL_TYPE, 0xC06, METHOD_BUFFERED, FILE_ANY_ACCESS)	// 40003018
#define IOCTL_UM_PROC_CLEAR		CTL_CODE(SIOCTL_TYPE, 0xC07, METHOD_BUFFERED, FILE_ANY_ACCESS)	// 4000301c
#define REQUEST_CONTENT_LENGTH	(1024 * 2)
#define REPLY_CONTENT_LENGTH	(1024 * 2)
...
typedef struct _REQUEST_PACKET
{
	ULONG	ulVersion;
	ULONG	ulType;
	CHAR	chContent[REQUEST_CONTENT_LENGTH];
} REQUEST_PACKET, *PREQUEST_PACKET, *LPPREQUEST_PACKET;

typedef struct _REPLY_PACKET
{
	ULONG	ulVersion;
	ULONG	ulValidContentSizeB;
	CHAR	chContent[REPLY_CONTENT_LENGTH];
} REPLY_PACKET, *PREPLY_PACKET, *LPREPLY_PACKET;

#define REQUEST_PACKET_SIZE		sizeof(REQUEST_PACKET)
#define REPLY_PACKET_SIZE		sizeof(REPLY_PACKET)
